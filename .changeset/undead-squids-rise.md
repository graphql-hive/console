...
Set the `CLICKHOUSE_OPERATIONS_V01_ROLLUPS_START` environment variable on the `server` container to an ISO timestamp after you successfully ran the latest clickhouse database migration in order to gradually start using the new tables.
...
A follow up breaking change release that will contain a database migration for dropping the tables `operations_minutely`, `operations_hourly`, `clients_minutely` and `clients_hourly` can be expected to land roughly ~60 days after this version is released. By then the new tables can satisfy all the date ranges for queries executed against those tables.

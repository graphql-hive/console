...
class CircuitBreaker extends EventEmitter {
  ...
  constructor(action, options = {}) {
    ...
    this.options.coalesceResetOn =
      options.coalesceResetOn?.filter(o => ['error', 'success', 'timeout'].includes(o)) || [];
    ...
    if (this.options.cache) {
      if (this.options.cacheTransport === undefined) {
        ...
        typeof this.options.cacheTransport !== 'object' ||
  ...
  call(context, ...rest) {
    ...
    const cacheKey =
      this.options.cache || this.options.coalesce ? this.options.cacheGetKey.apply(this, rest) : '';

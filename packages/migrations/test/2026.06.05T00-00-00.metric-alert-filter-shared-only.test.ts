...
await describe('migration: metric-alert-filter-shared-only', async () => {
  await test('detaches private-filter refs, keeps shared, and RESTRICTs deleting an in-use filter', async () => {
    ...
    try {
      ...
      // 3. The shared filter is in use -> ON DELETE RESTRICT blocks deletion at the
      //    DB layer (the hard backstop behind the API's friendly pre-check).

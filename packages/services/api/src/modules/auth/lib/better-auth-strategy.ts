...
import { AuthInstance } from '../providers/auth-instance';
...
export class BetterAuthCookieBasedSession extends Session {
  ...
  public async getActor(): Promise<UserActor> {
    const user = await this.storage.getUserByBetterAuthId({
      betterAuthUserId: this.betterAuthUserId,
    });
...
export class BetterAuthUserAuthNStrategy extends AuthNStrategy<BetterAuthCookieBasedSession> {
  ...
  private async verifyBetterAuthSession(args: { req: FastifyRequest; reply: FastifyReply }) {
    this.logger.debug('Attempt verifying Better Auth session');
    ...
    this.logger.debug('Better Auth session resolved.');
    return {
  ...
  async parse(args: {
    ...
    this.logger.debug('Better Auth session resolved successfully');

...
describe('refreshIamAuth', () => {
  describe('standalone mode', () => {
    it('calls AUTH and updates options.password', async () => {
      ...
      await refreshIamAuth(mockRedis, 'new-token-xyz', 'default', false, logger);
    ...
    it('rejects when AUTH fails on standalone Redis', async () => {
      ...
      await expect(
        refreshIamAuth(mockRedis, 'bad-token', 'default', false, logger),
      ).rejects.toThrow('WRONGPASS');

      // Password should NOT be updated on failure
      expect(mockRedis.options.password).toBe('old-token');
    });
  });
  ...
  describe('cluster mode', () => {
    it('iterates all cluster nodes and re-AUTHs each one', async () => {
      ...
      const mockCluster = {
        ...
        connectionPool: { redisOptions: { password: 'old' } },
      } as any;

      await refreshIamAuth(mockCluster, 'new-cluster-token', 'myuser', true, logger);
      ...
      expect(mockCluster.connectionPool.redisOptions.password).toBe('new-cluster-token');
    });

    it('updates connectionPool.redisOptions so new node connections use the fresh token', async () => {
      const { refreshIamAuth } = await import('./iam-redis');
      const logger = createMockLogger();

      const pool = { redisOptions: { password: 'old-password' } };
      const mockCluster = {
        nodes: vi.fn().mockReturnValue([]),
        connectionPool: pool,
      } as any;

      await refreshIamAuth(mockCluster, 'refreshed-token', 'user', true, logger);

      expect(pool.redisOptions.password).toBe('refreshed-token');
    });
    ...
    it('continues re-AUTHing remaining nodes when one node fails', async () => {
      ...
      await refreshIamAuth(mockCluster, 'token', 'default', true, logger);

      expect(healthyNode.options.password).toBe('token');
      expect(failingNode.options.password).toBe('old'); // Not updated on failure
    ...
    it('handles missing connectionPool gracefully', async () => {
      const { refreshIamAuth } = await import('./iam-redis');
      const logger = createMockLogger();
      ...
      await expect(
        refreshIamAuth(mockCluster, 'token', 'user', true, logger),
      ).resolves.toBeUndefined();
    });

    it('handles missing nodes() method gracefully', async () => {
      const { refreshIamAuth } = await import('./iam-redis');
      const logger = createMockLogger();
...
describe('startIamTokenRefresh', () => {
  ...
  describe('refresh lifecycle', () => {
    it('generates a token and re-AUTHs Redis on each tick', async () => {
      ...
      const timer = startIamTokenRefresh(mockRedis, config, false, logger);

...
export type Encryptor = {
  ...
  decrypt(text: string, possiblyRaw?: boolean): string;
};
 ...
 */
export function createEncryptor(encryptionSecret: string): Encryptor {
  const secret = crypto.createHash('md5').update(encryptionSecret).digest('hex');

  return {
    encrypt(text) {
      const secretBuffer = Buffer.from(secret, 'latin1');

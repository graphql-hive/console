 ...
 *
 * On failure the timer retries up to {@link TokenRefreshTimerOptions.maxRetries | maxRetries}
 ...
 * @returns A `setInterval` handle that can be passed to `clearInterval` to stop
 *   the refresh loop (e.g. during graceful shutdown).
 */
export function startTokenRefreshTimer(
  ...
  const jitterMs = Math.floor(Math.random() * (options?.jitterMs ?? 30_000));

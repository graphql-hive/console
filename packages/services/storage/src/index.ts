...
export async function createStorage(
  ...
  const storage: Storage = {
    ...
    async getSavedFilter(args) {
      const result = await pool.maybeOne(sql`/* getSavedFilter */
        SELECT
          "id"
      ...
      return SavedFilterModel.parse(result);
    },
    ...
    async getPaginatedSavedFiltersForProject(args) {
      ...
      const result = await pool.any(sql`/* getPaginatedSavedFiltersForProject */
        SELECT
          "id"
          , "project_id" as "projectId"
          , "type"
        ...
        WHERE
          ...
          ${
            args.visibility
              ? sql`AND "visibility" = ${args.visibility}`
              : sql``
          }
          ${
            args.search
      ...
      let items = result.map(row => {
        const node = SavedFilterModel.parse(row);
    ...
    async createSavedFilter(args) {
      const result = await pool.one(sql`/* createSavedFilter */
    ...
    async updateSavedFilter(args) {
      const result = await pool.maybeOne(sql`/* updateSavedFilter */
        UPDATE
          "saved_filters"
        SET
    ...
    async deleteSavedFilter(args) {
      const result = await pool.maybeOneFirst(sql`/* deleteSavedFilter */
        DELETE
        FROM
          "saved_filters"
        WHERE
    ...
    async incrementSavedFilterViews(args) {
      await pool.query(sql`/* incrementSavedFilterViews */
        UPDATE
          "saved_filters"
        SET

  ...
  return {
    ...
    init(routes, ctx) {
      routes.get('/authorize', async (req, reply) => {
        ...
        await ctx.cookie.set<ProviderState>(
          {
            req,
            reply,
          },
        ...
        authorization.searchParams.set('redirect_uri', getRelativeUrl(req, './callback'));
        ...
        console.log('redirecting to ', authorization.toString());
        reply.redirect(authorization.toString());
        return;
      });

      routes.get('/callback', async (req, reply) => {
        const provider = (await ctx.cookie.get({ req, reply }, 'provider')) as ProviderState;
        const queryParams = CallbackQuerySchema.parse(req.query);

        const code = queryParams.code;
        const state = queryParams.state;
        const error = queryParams.error;
        if (error)
          throw new OauthError(error.toString() as any, queryParams.error_description || '');
        if (!provider || !code || (provider.state && state !== provider.state)) {
          // redirect to proper page with error
          throw new Error('Eh, no code, no provider or state not in sync');
        }

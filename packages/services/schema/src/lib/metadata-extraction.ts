...
import { ConstDirectiveNode, FieldDefinitionNode, Kind, NameNode, StringValueNode, visit, type DocumentNode } from "graphql";

export const extractMetadata = (documentAst: DocumentNode): Record<string, Array<{ name: string, content: string }>> => {
  const schemaCoordinateMetadataMappings: Record<string, Array<{ name: string, content: string }>> = {};
...
export const mergeMetadata = (...subgraphs: Record<string, Array<{ name: string, content: string }>>[]) => {

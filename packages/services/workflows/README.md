...
| `EMAIL_PROVIDER_SMTP_REJECT_UNAUTHORIZED` | No                                                    | Whether the smtp server certificate must be valid. Set to `0` to allow self-signed certificates.                           | `1` (default) or `0`                                 |
| `EMAIL_PROVIDER_SMTP_IGNORE_TLS`          | No                                                    | Set to `1` to never use STARTTLS, even if the smtp server advertises support for it.                                       | `0` (default) or `1`                                 |
...
| `ENCRYPTION_SECRET`                       | No                                                    | Secret for decrypting stored secrets. Must match the API service. Slack metric-alert notifications are skipped without it. | `8ebe95cg21c1fee355e9fa32c8c33141`                   |

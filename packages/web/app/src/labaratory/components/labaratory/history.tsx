...
import type { LabaratoryHistory, LabaratoryHistoryRequest } from '@/labaratory/lib/history';
import { cn } from '@/labaratory/lib/utils';

export const HistoryOperationItem = (props: { historyItem: LabaratoryHistoryRequest }) => {
...
export const HistoryGroup = (props: { group: { date: string; items: LabaratoryHistory[] } }) => {
    ...
    <Collapsible open={isOpen} onOpenChange={setIsOpen}>
      ...
      <CollapsibleContent className={cn('border-border ml-4 flex flex-col gap-1 border-l pl-2')}>
        {props.group.items.map(h => {
          return <HistoryOperationItem key={h.id} historyItem={h as LabaratoryHistoryRequest} />;
        })}
      </CollapsibleContent>
    </Collapsible>
  );
...
export const History = () => {
  const { history, deleteAllHistory, tabs, setTabs, setActiveTab } = useLabaratory();

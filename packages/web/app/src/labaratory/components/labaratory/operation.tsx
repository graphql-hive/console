...
import { z } from 'zod';
import { Builder } from '@/labaratory/components/labaratory/builder';
...
import { Tabs } from '@/labaratory/components/tabs';
import { Badge } from '@/labaratory/components/ui/badge';
...
import { Field, FieldGroup, FieldLabel } from '@/labaratory/components/ui/field';
import {
  ResizableHandle,
  ResizablePanel,
  ResizablePanelGroup,
...
import {
  ...
  SelectTrigger,
  SelectValue,
} from '@/labaratory/components/ui/select';
import { Spinner } from '@/labaratory/components/ui/spinner';
import { Toggle } from '@/labaratory/components/ui/toggle';
import type {
  LabaratoryHistory,
...
const Variables = (props: { operation?: LabaratoryOperation | null; isReadOnly?: boolean }) => {
...
const Headers = (props: { operation?: LabaratoryOperation | null; isReadOnly?: boolean }) => {
  const { activeOperation, updateActiveOperation } = useLabaratory();
...
const Extensions = (props: { operation?: LabaratoryOperation | null; isReadOnly?: boolean }) => {
  const { activeOperation, updateActiveOperation } = useLabaratory();
...
export const ResponseBody = ({ historyItem }: { historyItem?: LabaratoryHistory | null }) => {
  return (
    <Editor
...
export const ResponseHeaders = ({ historyItem }: { historyItem?: LabaratoryHistory | null }) => {
  return (
    <Editor
      value={JSON.stringify(
...
export const ResponsePreflight = ({ historyItem }: { historyItem?: LabaratoryHistory | null }) => {
  return (
...
export const Response = ({ historyItem }: { historyItem?: LabaratoryHistoryRequest | null }) => {
        ...
        historyItem ? (
          <div className="ml-auto flex items-center gap-2 pr-3">
            ...
            <Badge variant="outline" className="bg-card">
              ...
              <span>
                {Math.round((historyItem as LabaratoryHistoryRequest).size / 1024)}
                KB
              </span>
            </Badge>
          </div>

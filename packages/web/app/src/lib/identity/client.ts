import { createLocalJWKSet, decodeJwt, errors, JSONWebKeySet, jwtVerify } from 'jose';
import { generatePKCE } from 'packages/services/server/src/identity/pkce.js';
import {
    ...
    async verify<T extends SubjectSchema>(
      ...
      try {
        const result = await jwtVerify<{
          mode: 'access';
          type: keyof T;
          properties: unknown; // v1.InferInput<T[keyof T]>;
        }>(token, jwks, {
          issuer,
        });
        return {
          aud: result.payload.aud as string,
          subject: result.payload.properties as any,
        };

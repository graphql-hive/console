...
Use the `Query.target` field to retrieve a target either by it's `id` or `slug` selector.
...
## Rertieve a List of Targets within a Project

Use the `Projetc.targets` field to retrieve a list of targets within the organization. **Note:**

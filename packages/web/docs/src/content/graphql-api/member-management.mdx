...
Organization members, roles, and invites can be managed via the GraphQL API.
...
assigning a member role to an user.
...
Use the `Mutation.assignMemberRole` field for assigning a member role to an user.
...
Use the `Mutation.deleteOrganizationInvitation` field for deleting a organization invitation.

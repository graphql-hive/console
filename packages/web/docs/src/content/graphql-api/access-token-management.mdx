---
title: Acess Token Management via the GraphQL API
...
actions available via the GraphQL API.

## Retrieve a List of Access Tokens

Use the `Organization.accessTokens` field for retreiving a paginated list of available access
tokens.
...
Instead of fetching a paginated list, it is also possible to retrieve a single access token by it's
...
A access token can be created by using the `Mutation.createOrganizationAccessToken` field.

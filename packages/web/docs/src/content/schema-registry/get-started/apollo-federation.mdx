...
import { Cards, FileTree, Steps, Tabs } from '@theguild/components'
...
<Callout type="info" title="What is Target ID?">
...
endpoint of your Hive target | | `<hive_cdn_access_key>` | The Hive CDN access key | |
`<hive_usage_target>` | The registry target which the usage data should be reported to | |
`<hive_registry_token>` | The Hive registry token for usage reporting | ======= | Parameter |
Description | | ----------------------- | ------------------------------------------- | |
`<target_id>` | The ID of your Hive target | | `<hive_cdn_access_key>` | The Hive CDN access key | |
`<hive_registry_token>` | The Hive registry token for usage reporting |

> > > > > > > 27f48402b (update endpoint docs)
...
```diff filename="router.config.yaml"
supergraph:
  type: hive
...
<Cards>
  ...
  <Cards.Card arrow title="Learn about Hive Gateway (JavaScript)" href="/docs/gateway" />
  <Cards.Card arrow title="Learn about Hive Router (Rust)" href="/docs/router" />
  <Cards.Card
    arrow

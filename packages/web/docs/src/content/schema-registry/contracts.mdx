...
If an element is removed due to tag filtering:

Fields referencing removed types may also need to be removed.

Otherwise, the resulting schema may become invalid.

If the final schema is invalid, **contract composition fails with an error**.

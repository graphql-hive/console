...
```bash filename="Retire an App Deployment via CLI"
hive app:retire \
  --registry.accessToken "<ACCESS_TOKEN>" \
  --target "<ORGANIZATION>/<PROJECT>/<TARGET>" \
  --name "my-app" \
...
Apollo Server automatically uses the server's [cache backend](https://www.apollographql.com/docs/apollo-server/performance/cache-backends) for L2 caching if available. You can

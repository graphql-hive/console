...
underlying storage and access tokens. Hive SDKs provide the option to configure a dual endpoint and have a built-in [circuit breaker](https://en.wikipedia.org/wiki/Circuit_breaker_design_pattern) that ensures a smooth experience in case the main CDN becomes unavailable.
...
Currently you can access the single services, merged schema, supergraph, metadata (e.g. GraphQL
Mesh artifacts) and persisted documents via the CDN.

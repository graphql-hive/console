...
enable the option to "Fail Checks for Dangerous Changes". This setting can be found under
the target's Settings "Breaking Changes" tab. Enabling this will cause dangerous changes to require
[Breaking change approval](/docs/schema-registry#approve-breaking-schema-changes) in order to publish.

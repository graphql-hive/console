...
enable the option to "Consider Dangerous Changes to be Breaking". This setting can be found under
the target's Settings "Breaking Changes" tab. Enabling this will cause dangerous changes to be
flagged as breaking, which will require
[Breaking change approval](/docs/schema-registry#approve-breaking-schema-changes) to publish.

...
# Hive JavaScript Client Reference

The JavaScript SDK is a set of libraries and utilities for interacting with Hive, for
[Usage Reporting](../schema-registry/usage-reporting.mdx), supergraph polling and persisted
documents.
...
Configure the circuit breaker for temporarily suspending using a CDN endpoint in case of network issues
or outages to avoid exhausting the service from with failing HTTP requests.

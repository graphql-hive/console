...
dependencies, traits, and structs, to simplify plugin development and keep the Router core version
up to date.
...
more information about how to implement a mutable state.
...
The following snippet shows the simplest example of creating a custom plugin, register it, and configure it with Hive Router: 
...
async fn main() -> Result<(), RouterInitError> {
    ...
    // Start and run the router entrypoing with your plugin
    router_entrypoint(
...
This hook is called after the deserialization of the request, and before the router has parsed the
GraphQL parameters body expected by
...
This hook is invoked during the Federation query planning process. At this stage, the Router already
extracted, parsed and validated the GraphQL operation, and it is ready to plan how to execute the
operation across subgraphs.
...
This hook is invoked during the Federation query execution process. At this stage, the Router
already extracted, parsed and validated the GraphQL operation. Also, a query plan has been
...
- `end_with_response_body(body: Serialize, status_code: StatusCode)` - Accepts a
  `serde`-serializable body, automatically serializes it to JSON, and sets the appropriate headers.
...
When creating custom plugins, it's important to understand that your plugin will be part of the request's hotpath.  The hotpath refers to the critical path that affects the performance of the router.

When implementing hooks in your plugin, you should be mindful of the performance implications of your code. 

Avoid doing heavy computations or blocking operations in request lifecycle hooks, if possible, as they can significantly impact the latency of your GraphQL requests.

Prefer [loading data in background](#background-tasks) and cache it if possible, instead of fetching during a GraphQL request workflow.

### Background Tasks

Hive Router provides a simple mechanism to run background tasks, using the [`tokio` runtime](https://docs.rs/tokio/latest/tokio/). 

As a plugin developer, you don't need to deal with threads or async runtimes - you can use the Hive Router core runtime in order to register and manage the lifecycle of your background tasks and threads. 

Background tasks can be registered during the [`on_plugin_init` hook](#on_plugin_init). Here's a simple example for a background tasks that runs periodically: 

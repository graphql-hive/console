...
Prefer [loading data in the background](#background-tasks) and caching it when possible, instead of fetching it during a GraphQL request.
...
Background tasks can be registered during the [`on_plugin_init` hook](#on_plugin_init). Here is a simple example of a background task that runs periodically:

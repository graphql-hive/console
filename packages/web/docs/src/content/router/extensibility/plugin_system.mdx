...
The configuration type requires to implement `DeserializeOwned + Sync` since the Router will be in charge of loading and deserializing the configuration from the Router config file. 
...
Hooks allow the developer to be part of the control-flow and change the behavior of the router, by returning different return types from the hook function.
...
This hook allows you to implement different strategies for handling the detection and the extraction of the GraphQL request parameters. You may also prevent the execution of the GraphQL request if certain conditions are met.
...
Plugins using this hook can have access to the Router's execution process, implement solutions like caching and other flows that replace the execution process as a whole. 
...
Hive Router is using the [`tracing` Crate](https://docs.rs/tracing/latest/tracing/) for collecting, processing and exporting OpenTelemetry trace information.

You may use `tracing` API to [create spans](https://docs.rs/tracing/latest/tracing/index.html#spans) and enrich the exported information with custom spans:
...
fn on_http_request<'req>(
    ...
    // Now, start it. The returned guard will make sure to finalize
    // and end the span when you hook function ends

...
Your [Registry Access Token](/docs/management/targets#registry-access-tokens) with write permission.
...
Explicitly enables usage reporting. Credentials (`token`/`target`) alone are not enough.
...
If set to `true`, the Hive Router will send traces to Hive Console.
...
Hive Console's traces ingestion endpoint.

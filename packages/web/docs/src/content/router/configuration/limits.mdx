...
The maximum allowed number of aliases in incoming GraphQL queries.
...
In that example, any incoming GraphQL query that contains more than 3 aliases will be rejected with
an error.
...
The above query contains 2 aliases (`user1` and `user2`), so it would be accepted. If a query

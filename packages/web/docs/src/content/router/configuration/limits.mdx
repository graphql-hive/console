...
In that example, any incoming GraphQL operation that contains more than 5 directives will be rejected
with an error.
...
The above query contains 4 directives, so it would be accepted. If an operation contained more than 5
...
In that example, any incoming GraphQL operation that contains more than 10 tokens will be rejected with
an error.
...
The above operation contains 8 tokens, so it would be accepted. If an operation contained more than 10
...
In that example, any incoming GraphQL operation that contains more than 3 aliases will be rejected with
an error.
...
The above operation contains 2 aliases (`user1` and `user2`), so it would be accepted. If an operation
contained more than 3 aliases, it would be rejected.

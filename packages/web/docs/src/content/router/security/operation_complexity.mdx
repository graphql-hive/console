...
implement operation complexity limits in your GraphQL router configuration.
...
performance of your GraphQL API server.
...
The above operation has a depth of 3 (`user` -> `posts` -> `comments`), so it would be accepted. If
an operation exceeded a depth of 10, it would be rejected.

...
Next step is to configure Hive Gateway to use [NATS](https://nats.io/) as the message broker and consume the
Mesh Compose generated supergraph. This is how the configuration would look like:

NATS PubSub does not come with Hive Gateway, you have to install the package and the
peer dependency of the chosen [NATS JavaScript transport](https://github.com/nats-io/nats.js) which you need to install first:
...
and then [publish an event to the NATS instance](https://docs.nats.io/nats-concepts/core-nats/pubsub/pubsub_walkthrough) on the `new_product` topic with the `edfs` prefix like
this:
...
For distributed use-cases requiring high-performance messaging, you can use the NATS-based PubSub engine. [NATS is a lightweight, high-performance messaging system that's ideal for microservices architectures and distributed systems.](https://nats.io/)

Similar to Redis PubSub, when running multiple instances of Hive Gateway, using a distributed PubSub engine like NATS ensures all instances are aware of active subscriptions and can publish events to the correct subscribers.

NATS PubSub does not come with Hive Gateway, you have to install the package and the NATS PubSub transport dependency of `@nats-io/transport-node` for Node or Bun which you need to install first:
...
Now, with this setup, any instance of Hive Gateway using the same `subjectPrefix` will be able to share the same subscriptions through the NATS messaging system.

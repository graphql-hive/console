...
The most commonly used otel packages are available when using Hive Gateway with CLI, which means
...
behavior by using the `flushOnDispose` option.
...
you can either create your own tracer, or manually instantiate the opentelemetry plugin (see
Programmatic Usage).
...
retrieve the current OTEL context by either using the `context.openTelemetry.getActiveContext`
...
If you don't have access to the graphql context, manually instantiate the OpenTelemetry plugin (see
Programmatic Usage).
...
export const gateway = createGatewayRuntime({
  plugins: ctx => {
    ...
    return [
      ...
      useGenericAuth({
        resolveUserFn: ({ context }) => {
          // Retrieve current OTEL Context by passing an object containing the graphql `context`,
...
To get access to the current span and add a custom attribute, event or link, you need to manually
provide the current OpenTelemetry context. You can retrieve the current OTEL context by either using
the `context.openTelemetry.getActiveContext()` function, or the utility function
`getActiveContext({ context, request, executionRequest })`.

```ts filename="gateway.config.ts"
import { defineConfig } from '@graphql-hive/gateway'
import { openTelemetrySetup } from '@graphql-mesh/plugin-opentelemetry/setup'
// This package re-export official @opentelemetry/api package for ease of use
import { trace, hive } from '@graphql-mesh/plugin-opentelemetry/api'
import { AsyncLocalStorageContextManager } from '@opentelemetry/context-async-hooks'
  ...
  plugins: () => [
    useGenericAuth({
      resolveUserFn: ({ context }) => {
        ...
        const ctx = context.openTelemetry.getActiveContext();
        // From global utility function
        const ctx = hive.getActiveContext({ context })

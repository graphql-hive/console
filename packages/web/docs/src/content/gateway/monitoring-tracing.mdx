...
export const gateway = createGatewayRuntime({
  plugins: ctx => {
    ...
    return [
      ...
      useGenericAuth({
        resolveUserFn: ({ context }) => {
          // If you have access to the configuration context:
          const tracer = ctx.openTelemetry.tracer;
          // Otherwise, you can access to the tracer on the plugin instance itself
          const tracer = otelPlugin.tracer;

          // `startActiveSpan` will rely on the current context to parent the new span correctly
          // You can also use your own tracer instead of Hive Gateway's one.
          return tracer.startActiveSpan('users.fetch', (span) => {
            const user = await fetchUser(extractUserIdFromContext(context))
            span.end();
            return user
          })
...
retreive the current OTEL context by either using the `context.openTelemetry.getActiveContext`
function, or the plugin's method `getActiveContext()`.
...
export const gatewayConfig = defineConfig({
  ...
  plugins: () => [
    useGenericAuth({
      resolveUserFn: ({ context }) => {
        const ctx = context.openTelemetry.getActiveContext();
    ...
    return [
      ...
      useGenericAuth({
        resolveUserFn: ({ context }) => {
          ...
          const otelCtx = context.openTelemetry.getActiveContext();

          // Explicitly pass the parent context as the third argument.
          return context.openTelemetry.tracer.startActiveSpan('users.fetch', {}, otelCtx, (span) => {
            const user = await fetchUser(extractUserIdFromContext(context))
            span.end();
            return user
          })
...
export const gatewayConfig = defineConfig({
  ...
  plugins: () => [
    useGenericAuth({
      resolveUserFn: ({ context }) => {
        const ctx = context.openTelemetry.getActiveContext({ context });
    ...
    return [
      ...
      useGenericAuth({
        resolveUserFn: ({ context }) => {
          ...
          // the http `request` and/or the `executionRequest`. The appropriate context will be
          // found based on which of those properties are provided.
          const otelCtx = ctx.openTelemetry.getActiveContext({ context });
          const span = trace.getSpan(ctx)
...
`getExecutionRequestContext(executionRequest)` functions.

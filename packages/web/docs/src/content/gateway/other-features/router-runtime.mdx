...
Running Hive Gateway with the Router Runtime is **up to 3x faster per core** with **up to 2x reduced
latency**.
...
core** with **up to 2x reduced latency** compared the Default (Stitching) Runtime thanks to the
Rust-powered query planner and optimizations for it.

See the official benchmark results for
[constant vus](https://github.com/graphql-hive/graphql-gateways-benchmark/blob/main/constant-vus-over-time_result.md)
and
[ramping vus](https://github.com/graphql-hive/graphql-gateways-benchmark/blob/main/ramping-vus_result.md)

...
By default, Hive Gateway uses the [Stitching Runtime](https://the-guild.dev/graphql/stitching),
which is a pure JavaScript implementation designed for flexibility and ease of use. The Stitching
Runtime is well-suited for most applications, providing a robust and adaptable solution for GraphQL
federation.
...
| Custom plugins               | ✅        | ⚠️ No stitching hooks | All plugins, except those using [stitching hooks](#no-stitching-runtime-plugin-hooks), will work |
...
Based on our internal measurements and performance testing, the Router Runtime demonstrates
significant performance improvements over the Stitching Runtime with **up to 3x faster query
planning per core** thanks to the Rust-powered query planner.

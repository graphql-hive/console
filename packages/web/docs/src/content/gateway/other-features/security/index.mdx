...
<Callout type="info">Query Depth Limiting is enabled by default in Hive Gateway and set to 7.</Callout>
...
<Callout type="info">Query Token Limiting is enabled by default in Hive Gateway and set to 1000.</Callout>
...
maximum token count an operation is allowed to have based on your schema and needs, as it could
vary between users.

...
> state externally. The gateway then decides whether to activate the override based on the flag
> value dynamically per request.
> [See the example here.](https://github.com/graphql-hive/gateway/tree/main/examples/launchdarkly-override)
...
The `progressiveOverride` function can also be **async** and return a Promise. This allows you to
perform asynchronous operations like fetching from external services, database queries, or other
async logic to determine if a label should be active:
...
export const gatewayConfig = defineConfig({
  async progressiveOverride(label: string, context: GatewayContext) {
    if (label === 'use-fulfillment-service') {
      // Fetch from an external service to determine if override should be active
      const response = await fetch('https://api.myfeaturechecker.com/feature-flags', {
        headers: {
          'x-user-id': context.request.headers.get('x-user-id') || ''
        }
      })
...
- We fetch feature flags from an external API service
- We pass the user ID from request headers to the service
- The service responds with feature flags, and we check if `useFulfillmentService` is enabled
- The function returns a Promise that resolves to a boolean, determining whether the override is
  active

...
- [Renamed CLI options for Hive Usage Report](#hive-usage-cli-options)
...
<div>
  OpenTelemetry <code>NodeSDK</code>
</div>
]}>
...
### Opentelemetry CLI Options

It is now possible to setup opentelemtry without a configuration file, by using the new
...
## Hive Usage CLI Options

To prepare for future observability features of Hive Console, some CLI options have been renamed:

- `--hive-usage-target` have been renamed `--hive-target`
- `--hive-access-token` is a new option to define an access token that will be used for all
  observability features.
- `--hive-usage-access-token` is now a specific access token that will be used only for usage
  report.

...
#### Example using HTTP OTLP Exporter
...
To prepare for future observability features of Hive Console, `--hive-usage-target` has been renamed
to `--hive-target` which will define the target for the observability metrics _and_ usage reporting.
...
Please note that `--hive-usage-access-token` stays, and is now a specific access token that will be
used **only** for usage reporting. So if you're only reporting usage metrics to Hive Console, you
can keep using the `--hive-usage-access-token` option:

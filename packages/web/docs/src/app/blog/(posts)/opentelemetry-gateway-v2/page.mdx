---
title: Monitoring Hive Gateway with OpenTelemetry
tags: [graphql, monitoring, opentelemetry, hive]
authors: valentin
date: 2025-09-04
description: Explore the new OpenTelemetry integration for Hive Gateway v2
---
...
We're excited to announce significant improvements to Hive Gateway's OpenTelemetry capabilities! For
detailed information about implementing and configuring these features, please refer to the
[Hive Gateway v2 blog post →](/blog/hive-gateway-v2)

While OpenTelemetry is becoming a widely used solution, we listened to many of production users's
feedback and build the best GraphQL integration possible.
...
`openTelemetrySetup` function from `@graphql-hive/plugin-opentelemetry/setup` or we support standard
`NodeSDK` from `@opentelemetry/sdk-node` for a NodeJS specific setup.

<Tabs items={[<span key="cli">CLI</span>, <code key="setup">openTelemetrySetup</code>, <code key="node">NodeSDK</code>]}>

<Tabs.Tab>

```bash
hive-gateway supergraph --opentelemetry [exporter-otlp-endpoint]
```
...
<video>
  <source src="/blog-assets/opentelemetry-gateway-v2/demo-span-parenting.webm" type="video/webm" />
  <source src="/blog-assets/opentelemetry-gateway-v2/demo-span-parenting.mp4" type="video/mp4" />
  <a
    alt="Click on this image to download video demo"
    href="/blog-assets/opentelemetry-gateway-v2/demo-span-parenting.mp4"
  >
    <img src="/blog-assets/opentelemetry-gateway-v2/demo-span-parenting.png" />
  </a>
...
![A screenshot demoing batched graphql operation tracing](./demo-http-batching.png)
...
![A screenshot demoing failed and retried upstream request with appropriate attribute](./demo-http-retry.png)
...
![A screenshot demoing support of standard fetch instrumentation](./demo-instrumentation.png)
...
export const gatewayConfig = defineConfig({
  ...
  genericAuth: {
    ...
    resolveUserFn: ctx =>
      hive.tracer!.startActiveSpan('auth.validate_user', async span => {
...
![A screenshot showing custom span registered at context building time](./demo-custom-span.png)
...
export const gatewayConfig = defineConfig({
  ...
  genericAuth: {
    ...
    async resolveUserFn(ctx) {
      ...
      const httpSpan = trace.getSpan(hive.getHttpContext(ctx.request))
...
![A screenshot showing an example of custom attribute](./demo-custom-attributes.png)
...
import './telemetry.ts'
import { defineConfig } from '@graphql-hive/gateway'
import { context, hive, propagation } from '@graphql-hive/gateway/opentelemetry/api'
  ...
  plugins: () => [
    {
      instrumentation: {
        request({ request }, wrapped) {
          ...
          const ctxWithBaggage = propagation.setBaggage(hive.getActiveContext({ request }), baggage)
...
The new OpenTelemetry integration leverages the new [Hive Logger](/docs/logger) feature with an
integration with the logger API of OpenTelemetry.
...
use `@graphql-hive/plugin-opentelemetry` to improve your GraphQL server observability and take
...
To learn more about Hive Gateway's OpenTelemetry integration or to get started with monitoring,
visit our [documentation](/graphql/hive/docs/gateway).

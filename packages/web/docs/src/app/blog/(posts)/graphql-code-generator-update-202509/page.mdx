...
date: 2025-09-07
description:
  GraphQL Code Generator September 2025 update comes with improvements to Federation and monolithic
  GraphQL server typing and lots of internal quality-of-life changes.
---

GraphQL Code Generator has just gone through a major update to improve type usability for both
Federation and monolithic GraphQL server architecture.
...
- Federation `__resolveReference` is available for Interfaces
- Fixed Federation type generation when `@external`, `@requires`, etc. are used.
- Interfaces types having non-meta resolvers e.g. `__resolveReference`
...
- Deprecated `generateInternalResolversIfNeeded` option. Codegen decides whether to generate
  internal resolvers to match the spec
...
This GraphQL Codegen major version update comes with improved support for Federation and monolithic
use cases for type correctness based on community feedback and collaboration. It also overhauls old
config and internal to improve contributor's Developer Experience. Please create issues

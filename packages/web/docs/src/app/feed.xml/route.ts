...
export async function GET() {
  ...
  const outputContent = `// THIS CODE WAS AUTOMATICALLY GENERATED BY @hive/docs in src/app/feed.xml/route.ts

export const latestChangelog = ${JSON.stringify(latestChangelog, null, 2)};
`;
  await fs.writeFile(outputFilePath, outputContent, 'utf8');
  console.info(
    `Changelog generated successfully at: ${path.relative(process.cwd(), outputFilePath)}`,
  );

...
3. **Gateway Evolution**
   ...
   Wealthsimple is improving their its API gateway infrastructure by focusing on migrating from
   their current Apollo server with schema stitching to Hive Gateway. Having already tested

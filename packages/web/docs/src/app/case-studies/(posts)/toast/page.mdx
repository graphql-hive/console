...
<Table scheme='neutral' className="mt-6 text-sm  dark:[--border:theme(colors.beige.900/.75)] case-studies-breakout max-sm:rounded-none max-sm:border-x-0 [&_td:first-of-type]:font-medium [&_li:not(:first-of-type)]:mt-0 [&_td:has(>ul)]:p-2">
  <tbody>
    ...
    <Table.Row>
      ...
      <Table.Cell>
        Toast developed their own tool for efficient local development called "PrepStation". The tool deeply integrates with Hive Console, Storybook, GraphQL Codegen and Yoga Server to
        ...
        do any work. The Toast team gave a [great

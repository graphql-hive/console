export const metadata = {
  ...
  excerpt:
    "How Toast achieved increased productivity, direct developer support, long-term stability, cost reduction and no vendor lock-in with The Guild's open source GraphQL stack.",
...
import afterDiagram from './after-diagram.svg'
import architectureDiagram from './architecture-diagram.svg'
import beforeDiagram from './before-diagram.svg'

export const Diagram = ({ src }) => (
  <div className="bg-green-1000 -mx-6 mt-6 flex w-[calc(100%+3rem)] p-6 max-sm:w-screen sm:rounded-2xl">
    <Image src={src} alt="" role="presentation" className="!rounded-none" />
  </div>
)
...
Federation would become load-bearing infrastructure — the thing every team builds on, every request
flows through. Pick wrong, and you're stuck with technical debt that outlasts the people who created
it
...
<Diagram src={architectureDiagram} />
...
1. Reliability – Hive Console provided the stability Toast needed at scale
...
6. [Security](https://security.graphql-hive.com/) and compliance were paramount for Toast. Hive's
...
## Gateway and subgraphs – Why Toast chose Hive

When evaluating GraphQL Federation, Toast tested Apollo's gateway and subgraphs on one project.
During that evaluation, they identified concerns that led them to choose Hive instead:
...
From a business perspective, these concerns pointed to potential risks:

1. Stability risks – outdated components could become harder to rely on at scale.
...
The second phase is migrating from Apollo Server to GraphQL Yoga server by The Guild, which
...
Paid enterprise-exclusive features for Apollo users that Hive provides for free:
...
- **Business Support:** **24x7x365** technical support with top-tier SLAs (Service Level
  Agreements).
...
**Features Apollo offers for free – and Hive does too:**
...
- **Request Authorization & JWT Authentication:** Built-in security and access control features at
  the router level.
- **Traffic Shaping:** Features such as deduplication and rate limiting.
...
<Table scheme='neutral' className="mt-6 text-sm  dark:[--border:theme(colors.beige.900/.75)] case-studies-breakout max-sm:rounded-none max-sm:border-x-0 [&_td:first-of-type]:font-medium [&_li:not(:first-of-type)]:mt-0 [&_td:has(>ul)]:p-2">
  <tbody>
...
### New Hive Rust Router

The Guild is working hard on
[Hive Router](https://the-guild.dev/graphql/hive/blog/welcome-hive-router), their new GraphQL
Federation Rust Router.

It is already the [most correct](https://the-guild.dev/graphql/hive/federation-gateway-audit) and
the [most performant](https://the-guild.dev/graphql/hive/federation-gateway-performance) router in
the ecosystem.

Toast is interested in trying it out, waiting the flexibility of Hive Gateway JS.

An initial step would be to try the next upcoming version of Hive Gateway, which will include Hive
Router’s Rust query planner and has great performance improvements while keeping the gateway in
javascript still.
...
Toast's experience highlights the value of open, flexible GraphQL tooling for companies scaling
fast. Hive enabled them to modernize gradually, improve performance, and reduce costs while keeping
development efficient. Looking ahead, both teams continue working closely to support new needs and

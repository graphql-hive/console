...
excerpt:
  'Hemnet is Sweden’s largest property platform, serving millions of users who browse, save, and
  search through real estate listings every day. Behind the scenes, a GraphQL API handles the bulk
  of these interactions, powering everything from listing pages and search results to user accounts
  and saved properties. For years, this API ran on a monolithic GraphQL layer built on top of a
  Rails stack with PostgreSQL. As the platform grew, so did the pain points, particularly the
  difficulty of splitting concerns into smaller, independently maintainable chunks. But how do you
  introduce separation into a backend that was designed to operate as a single entity?'
...
Hemnet is Sweden’s largest property platform, serving millions of users who browse, save, and search
through real estate listings every day. Behind the scenes, a GraphQL API handles the bulk of these
interactions, powering everything from listing pages and search results to user accounts and saved
properties. For years, this API ran on a monolithic GraphQL layer built on top of a Rails stack with
PostgreSQL. As the platform grew, so did the pain points, particularly the difficulty of splitting
concerns into smaller, independently maintainable chunks. But how do you introduce separation into a
backend that was designed to operate as a single entity?
...
Rust-based router to a [Node.js](http://Node.js)\-based one with Hive would come with a measurable
...
[**Node.js**](http://Node.js)**\-based** gateway proved too slow, we could fall back to the Apollo
...
[Node.js](http://Node.js) gateway to consume significantly more CPU and memory since it's more
...
>
  Transform your GraphQL infrastructure like Hemnet. Whether you’re replacing legacy gateways,
  introducing federation, or improving schema governance, Hive gives you the tools to evolve safely,

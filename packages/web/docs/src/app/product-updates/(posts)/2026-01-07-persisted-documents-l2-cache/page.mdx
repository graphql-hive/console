...
The in-memory cache (L1) for persisted documents is lost when:

- Serverless functions cold start
- Server instances restart
- New instances are added during scaling

This results in repeated CDN calls for the same documents.

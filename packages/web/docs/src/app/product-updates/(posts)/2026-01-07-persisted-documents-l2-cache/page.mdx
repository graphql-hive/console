...
Apollo Server automatically uses `ctx.cache` as the L2 cache when available. Hive Gateway
automatically uses the gateway cache when TTL options are provided.

---
title: Easier Gradual Schema Migrations with Progressive Override
description:
...
We are excited to introduce support for progressive overrides in Hive Gateway and Hive Router,
...
supergraph remains the same. Progressive overrides allows you to make these changes gradually by
controlling the traffic of the affected fields.

### `@override` directive with traffic percentage

You can now use the `@override` directive along with a `label` argument and `percent(x)` value to
specify the percentage of traffic that should be routed to the new implementation of a field.
...
Now with progressive overrides, you can control the traffic percentage like in the following
example:
...
This configuration will route 20% of the traffic for the `amount` field to the `Billing` subgraph,
while the remaining 80% will still be routed by the `Payments` subgraph. This allows you to monitor
...
In addition to percentage-based traffic control, progressive overrides also supports using custom
labels to manage field overrides. This allows you to route traffic based on specific criteria or
conditions that are relevant to your application, e.g. toggleable feature flags or headers.
